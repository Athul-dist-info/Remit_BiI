...
class ForeignCurSaleWiseAnalysis extends StatefulWidget {
  const ForeignCurSaleWiseAnalysis({
    super.key,
    required this.title,
  ...
  @override
  State<ForeignCurSaleWiseAnalysis> createState() =>
      _ForeignCurSaleWiseAnalysisState();
}

class _ForeignCurSaleWiseAnalysisState
    extends State<ForeignCurSaleWiseAnalysis> {
  ...
  Future<void> loadRemittanceData() async {
    ...
    try {
      final responseData = await remittanceController
          .getForeignCurrencySaleRemittanceAnalysis(
            fromDate:
      ...
      legendItemsForPie = responseData.map((e) => e.curCode).toSet().toList();
